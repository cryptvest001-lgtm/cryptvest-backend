...
import { KycStatus, Network } from "@cryptvest/shared";
...
adminRouter.get("/users", async (_req: AuthRequest, res: Response) => {
  const users = await prisma.user.findMany({
    select: {
      id: true, email: true, role: true, kycStatus: true,
...
adminRouter.get("/reports", async (_req: AuthRequest, res: Response) => {
  const [userCount, totalDeposited, totalStaked, totalWithdrawn, pendingWithdrawals] =
    await Promise.all([
      prisma.user.count(),
...
adminRouter.get("/wallets", async (_req: AuthRequest, res: Response) => {
  ...
  const btcAddresses = addresses.filter((a: AddrRow) => a.network === Network.BTC);
  const ethAddresses = addresses.filter((a: AddrRow) => a.network === Network.ERC20);

  const btcResults = await Promise.all(
    btcAddresses.map(async (a: AddrRow) => {
      const bal = await getBtcAddressBalance(a.address).catch(() => ({ confirmed: 0n, unconfirmed: 0n }));
      return {
        address: a.address,
        network: Network.BTC,
        userId: a.userId,
  ...
  const ethResults = await Promise.all(
    ethAddresses.map(async (a: AddrRow) => {
      ...
      return { address: a.address, network: Network.ERC20, userId: a.userId, ethBal, usdtBal };
    })
  );
...
adminRouter.post("/kyc/:id/decision", async (req: AuthRequest, res: Response) => {
  ...
  const user = await prisma.user.findUnique({ where: { id: submission.userId }, select: { email: true } });
  if (user) {
